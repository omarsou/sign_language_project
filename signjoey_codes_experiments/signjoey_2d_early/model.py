...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        keypoints_embed: SpatialEmbeddings,
        ...
        self.keypoints_embed = keypoints_embed
    ...
    def forward(
        ...
        encoder_output, encoder_hidden = self.encode(
            sgn=sgn, sgn_mask=sgn_mask, keyp=keyp, keyp_mask=keyp_mask, sgn_length=sgn_lengths
        )
    ...
    def encode(
        self, sgn: Tensor, sgn_mask: Tensor, keyp: Tensor, keyp_mask: Tensor, sgn_length: Tensor
    ) -> (Tensor, Tensor):
        """
        ...
        """
        sng_embd = self.sgn_embed(x=sgn, mask=sgn_mask)
        keypoints_embd = self.keypoints_embed(x=keyp, mask=keyp_mask)
        src = cat([sng_embd, keypoints_embd], axis=-1)
        return self.encoder(
            embed_src=src,
    ...
    def get_loss_for_batch(
        ...
        decoder_outputs, gloss_probabilities = self.forward(
            ...
            keyp=batch.keypoints,
            keyp_mask=batch.keypoints_mask,
    ...
    def run_batch(
        ...
        encoder_output, encoder_hidden = self.encode(
            sgn=batch.sgn, sgn_mask=batch.sgn_mask, keyp=batch.keypoints, keyp_mask=batch.keypoints_mask,
...
def build_model(
    ...
    keypoints_embed: SpatialEmbeddings = SpatialEmbeddings(
        embedding_dim=keypoints_dim//2 + (8 - (cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//2)%8),
        scale=False,
        dropout=0.1,
        norm_type='batch',
        activation_type='softsign',
        num_heads=cfg["encoder"]["num_heads"],
        input_size=keypoints_dim,
    )
    ...
    if cfg["encoder"].get("type", "recurrent") == "transformer":
        assert (
            cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//2 + (8 - (cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//2)%8)
            == cfg["encoder"]["hidden_size"]
        ), "for transformer, emb_size must be hidden_size"

        encoder = TransformerEncoder(
            **cfg["encoder"],
            emb_size=cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//2 + (8 - (cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//2)%8),
    ...
    model: SignModel = SignModel(
        ...
        decoder=decoder,
        sgn_embed=sgn_embed,
        keypoints_embed=keypoints_embed,
        txt_embed=txt_embed,
        gls_vocab=gls_vocab,
        txt_vocab=txt_vocab,
