...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        features_embed: SpatialEmbeddings,
    ...
    def forward(
        ...
        """
        encoder_sgn_output,  encoder_features_output, _, _ = self.encode(
            sgn=sgn, sgn_mask=sgn_mask, features=features, features_mask=features_mask, sgn_length=sgn_lengths
        )
        
        encoder_output = self.alpha * encoder_sgn_output + self.gamma * encoder_features_output
        encoder_hidden = None
    ...
    def encode(
        self, sgn: Tensor, sgn_mask: Tensor, features: Tensor, features_mask: Tensor, sgn_length: Tensor
    ) -> (Tensor, Tensor, Tensor, Tensor):
        """
        ...
        features_embd = self.features_embed(x=features, mask=features_mask)
        encoder_sgn_output, encoder_sgn_hidden = self.encoder(
            embed_src=sng_embd,
            src_length=sgn_length,
            mask=sgn_mask)
        encoder_features_output, encoder_features_hidden = self.encoder_features(embed_src=features_embd,
                                 src_length=sgn_length,
                                 mask=features_mask)
        return encoder_sgn_output, encoder_features_output, encoder_sgn_hidden, encoder_features_hidden
    ...
    def run_batch(
        ...
        encoder_sgn_output, encoder_features_output, _, _  = self.encode(
            sgn=batch.sgn, sgn_mask=batch.sgn_mask, features=batch.features, features_mask=batch.features_mask,
...
def build_model(
    ...
    if cfg["encoder"].get("type", "recurrent") == "transformer":
        ...
        encoder_features = TransformerEncoder(
            **cfg["encoder"],
            emb_size=cfg["encoder"]["embeddings"]["embedding_dim"],
            emb_dropout=enc_emb_dropout,
        )
