...
class TransformerEncoder(Encoder):
    ...
    # pylint: disable=arguments-differ
    def forward(
        self, x: Tensor, memory: Tensor, memory_mask: Tensor, src_mask: Tensor
    ) -> (Tensor, Tensor):
        """
        ...
        memory = self.emb_dropout(memory)

        for layer in self.layers:
            x = layer(x=x, memory=memory, memory_mask=memory_mask, src_mask=src_mask)
