...
def build_model(
    ...
    keypoints_embed: SpatialEmbeddings = SpatialEmbeddings(
        embedding_dim=keypoints_dim//3 + (8 - (cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//3)%8),
        scale=False,
        dropout=0.1,
        norm_type='batch',
    ...
    if cfg["encoder"].get("type", "recurrent") == "transformer":
        assert (
            cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//3 + (8 - (cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//3)%8)
            == cfg["encoder"]["hidden_size"]
        ), "for transformer, emb_size must be hidden_size"

        encoder = TransformerEncoder(
            **cfg["encoder"],
            emb_size=cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//3 + (8 - (cfg["encoder"]["embeddings"]["embedding_dim"] + keypoints_dim//3)%8),
